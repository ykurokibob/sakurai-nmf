...
% n_train = 500;
% n2_train = n_train/2;
% X_train = rand(2,n_train);
% noise = (rand(1,n_train)*2-1)*0.2;
% 
% Y_train = zeros(2,n_train);
% for i = 1:n_train
%     if X_train(1,i)^2 + X_train(2,i)^2 < 1+noise(i)
%         Y_train(1,i) = 1;
%     else
%         Y_train(2,i) = 1;
%     end
...
%param.nsnmf  = [10, 10];        % # iters for LSQs in nonlinear semi-NMF
param.nsnmf  = [1, 1];

param.batch  = [5000,5000];   % size of batch for AE and fine tuning
param.lambda = [1e-3,1e-5];
%param.lambda = [1e-2,1e-3];
%param.lambda = [0,0];
%param.lambda = 1e-2;
%
% Set parameters
...
rng(seed, 'twister');
disp(param)
[WZ,WZ_train,WZ_test,resvec(:,:,seed+1)] = myDeepNN_br_class(X_train,Y_train,X_test,Y_test,param);
