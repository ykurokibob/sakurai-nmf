function [WZ,WZ_train,WZ_test,resvec] = myDeepNN_class(A,Y,A_test,Y_test,param)
tic;
t0 = toc;
tc = 0;
fprintf('-- AutoEncoder ------------------------------------- \n');
...
for itr = 1:ftitr(1)
    ...
    for kk = 1:m/batch(2)
      ...
      ide = kk*batch(2);
		...
		l = l + 1;
		resvec(l,:) = check_class(itr,toc-t0,A,A_test,Y,Y_test,WZ,L);
...
WZ_train = compute_z(A,WZ,L);
WZ_test = compute_z(A_test,WZ,L);
