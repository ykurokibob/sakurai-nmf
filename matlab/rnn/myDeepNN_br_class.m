function [WZ,WZ_train,WZ_test,resvec] = myDeepNN_br_class(A,Y,A_test,Y_test,param)
tic;
t0 = toc;
tc = 0;
fprintf('-- AutoEncoder ------------------------------------- \n');
...
l = 1;
tc0 = toc;
resvec(l,:) = check_br_class(0,toc-t0,A,A_test,Y,Y_test,WZ,L); tc = tc + (toc-tc0);
...
for itr = 1:ftitr(1)
    ...
    for kk = 1:m/batch(2)
        ...
        %%---------------------------------------------------------
        l = l + 1;
        resvec(l,:) = check_br_class(itr,toc-t0,A,A_test,Y,Y_test,WZ,L);
        %if resvec
    end
    %     output = compute_z_br(A_test,WZ,L);
    %     for p_n = 1:param.n_test
    %         [~,id] = max(output(L).Z(:,p_n));
    %         if id == 1
    %
...
WZ_test = compute_z_br(A_test,WZ,L);
