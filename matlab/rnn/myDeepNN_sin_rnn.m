...
function [WZ,resvec] = myDeepNN_sin_rnn(A,Y,A_test,Y_test,param)
tic;
t0 = toc;
tc = 0;
fprintf('-- AutoEncoder ------------------------------------- \n');
...
fprintf('---------------------------------------------------- \n');
%for j = 1:10
WZ(1).Wre=rand(1)*(1e-03);
WZ(1).Win=rand(1)*(1e-03);
WZ(1).Wout=rand(1)*(1e-03);
%end
%WZ(1).Wre=1.1;
%WZ(1).Win=1.2;
WRe1=WZ(1).Wre
...
%     Err=Err;
    ...
    [WZ(1).Wout,Zall] = nmf_rnn('S_F',Y,WZ(1).Wout,Zall,lambda(1),lambda(2),ftitr(2),nsnmf(1),nsnmf(2),delta(2)); %***???X***
