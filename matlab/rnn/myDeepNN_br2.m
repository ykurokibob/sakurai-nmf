function [WZ,resvec] = myDeepNN_br2(A,Y,A_test,Y_test,param)
tic;
t0 = toc;
tc = 0;
fprintf('-- AutoEncoder ------------------------------------- \n');
...
WZ(1).W = autoencoder_br(1,AI,AIusv,param); %***変更***
a = ones(size(AI,2),1);
...
for i = 2:L-1
  ...
  WZ(i).W = autoencoder_br(i,WZ(i-1).Z,usv,param); %***変更***;
...
for itr = 1:ftitr(1)
  ...
  id = randperm(m);
    for kk = 1:m/batch(2)
      ...
      [WZ(L).W,WZ(L-1).Z] = nmf_br('S2',YI,WZ(L).W,WZ(L-1).Z, ...
                                   lambda(1),lambda(2),ftitr(2),0,0,0); %***変更***
        
        for i = L-2:-1:1
          %nmf縺悟他縺ｰ繧後◆縺ｨ縺阪?陦悟?繧剃ｿ晏ｭ倥☆繧?
          [WZ(i+1).W,WZ(i).Z] = nmf_br('NS2',WZ(i+1).Z,WZ(i+1).W, ...
                                       WZ(i).Z,lambda(1),lambda(2),ftitr(3),nsnmf(1),nsnmf(2),delta(2)); %***変更***
        end
        
        WZ(1).W = non_linear_lsq_br('XA2',AI,AIusv,WZ(1).Z,WZ(1).W,lambda(1),nsnmf(1)); %***変更***
