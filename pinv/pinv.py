...
def mat_pinv(a, b, rcond=1e-8, a_dagger=True):
    """
    :return: X = a^\dagger b
    """
    with tf.name_scope("tf_pinv"):
    ...
    with tf.device('/cpu:0'):
        if a_dagger:
            ...
            s, u, v = tf.svd(b, full_matrices=False)
        ...
        if a_dagger:
            res = tf.matmul(v,
                            tf.multiply(s[..., tf.newaxis],
                                        tf.matmul(tf.transpose(u),
                                                  b)))
        else:
            av = tf.matmul(a, v)
            avs = tf.multiply(av, s)
            res = tf.matmul(avs, tf.transpose(u))
        return res